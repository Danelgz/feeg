...
export function getTokens(isDark) {
  return {
    ...
    // Línea de 1px para separar secciones y filas SIN encerrarlas en una tarjeta. Translúcida y no
    // un gris fijo: se ve igual de sutil sobre el negro de la página que sobre una superficie.
    hairline: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)',
    text: isDark ? '#fff' : '#333',
    ...
    motion: MOTION,
    heat: HEAT,
    // Par categórico para comparar dos mitades (Empuje/Tirón, tren superior/inferior). Validado con
    // el script de la guía de dataviz: CVD ΔE ~21 y visión normal ~23 en ambos temas, cada paso
    // dentro de la banda de luminosidad de su superficie. Van siempre con etiqueta directa y hueco
    // de 2px entre segmentos (la separación tritan es baja, ~6), nunca solo por color.
    split: isDark ? ['#16a37f', '#5b7cf0'] : ['#0f9a78', '#4f6ee6'],
