import { initializeApp, getApps } from 'firebase/app';
import { getAuth, GoogleAuthProvider, signInWithPopup, signInWithRedirect, getRedirectResult, signOut, onAuthStateChanged } from 'firebase/auth';
import { getFirestore, doc, setDoc, getDoc, deleteDoc, updateDoc, collection, addDoc, query, getDocs, where, orderBy, onSnapshot, arrayUnion, arrayRemove, limit, startAfter, writeBatch, getCountFromServer } from 'firebase/firestore';
import { getStorage, ref, uploadString, getDownloadURL } from 'firebase/storage';

// TODO: Replace with your Firebase web config or set env vars
const firebaseConfig = {
  apiKey: process.env.NEXT_PUBLIC_FIREBASE_API_KEY,
  authDomain: process.env.NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN,
  projectId: process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID,
  storageBucket: process.env.NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET,
  messagingSenderId: process.env.NEXT_PUBLIC_FIREBASE_MESSAGING_SENDER_ID,
  appId: process.env.NEXT_PUBLIC_FIREBASE_APP_ID
};

// Initialize Firebase only if API key is present to avoid build-time errors
export const app = (firebaseConfig.apiKey && (getApps().length ? getApps()[0] : initializeApp(firebaseConfig))) || null;
// initializeAuth con persistencia explícita se probó para mejorar la fiabilidad del redirect
// en Safari/iOS, pero se revirtió: Next.js ejecuta este módulo también en el servidor (SSR),
// donde no existe indexedDB/window, y eso dejaba el auth en un estado inválido que rompía
// CUALQUIER llamada real (signInWithPopup, getRedirectResult) con "auth/argument-error" en
// producción. getAuth() simple es seguro tanto en servidor como en cliente.
export const auth = app ? getAuth(app) : null;
export const db = app ? getFirestore(app) : null;
export const storage = app ? getStorage(app) : null;
export const googleProvider = new GoogleAuthProvider();
// Sin esto, si el navegador/dispositivo ya tiene una única sesión de Google activa, Firebase
// la reutiliza directamente sin preguntar — el usuario puede terminar logueado con una cuenta
// que no quería. Forzamos el selector de cuenta siempre.
googleProvider.setCustomParameters({ prompt: 'select_account' });

// En redes móviles lentas/inestables, getDoc/getDocs de Firestore pueden quedarse pendientes
// mucho más tiempo del razonable en vez de fallar rápido — sin este timeout, cualquier lectura
// colgada bloqueaba isSyncing (y con él la pantalla completa de "Sincronizando tus datos..." en
// Layout.jsx) indefinidamente. saveToCloud ya tenía esta protección; withTimeout la comparte
// también con las lecturas (getFromCloud, getFollowersList) que antes no la tenían.
const FIRESTORE_TIMEOUT_MS = 10000;
const withTimeout = (promise, message = "Tiempo de espera agotado") =>
  Promise.race([
    promise,
    new Promise((_, reject) => setTimeout(() => reject(new Error(message)), FIRESTORE_TIMEOUT_MS)),
  ]);

let isAuthActionInProgress = false;

export const signInWithGoogle = async () => {
  if (!auth) {
    const errorMsg = 'Firebase Auth no está configurado. Verifica las variables de entorno.';
    console.error(errorMsg);
    return { error: { message: errorMsg, code: 'auth/not-configured' } };
  }

  if (isAuthActionInProgress) return null;
  isAuthActionInProgress = true;

  try {
    try {
      // Preferimos popup siempre: a diferencia de signInWithRedirect, no depende de un
      // traspaso de almacenamiento entre vuestro dominio y *.firebaseapp.com, que es justo lo
      // que la protección anti-tracking de Safari/iOS suele romper (el login se completa en
      // Google pero el resultado nunca vuelve a la app). Solo caemos a redirect si el propio
      // navegador bloquea el popup o no lo soporta.
      const res = await signInWithPopup(auth, googleProvider);
      return res.user;
    } catch (popupError) {
      // Ignorar error si el usuario simplemente cerró la ventana o canceló el popup
      const isCancelled = popupError.code === 'auth/popup-closed-by-user' ||
                          popupError.code === 'auth/cancelled-popup-request' ||
                          popupError.message?.includes('cancelled-popup-request') ||
                          popupError.message?.includes('popup-closed-by-user');
      if (isCancelled) {
        console.log('Autenticación cancelada por el usuario');
        return 'cancelled';
      }

      const shouldFallbackToRedirect =
        popupError.code === 'auth/popup-blocked' ||
        popupError.code === 'auth/operation-not-supported-in-this-environment';

      if (shouldFallbackToRedirect) {
        await signInWithRedirect(auth, googleProvider);
        return 'redirecting';
      }

      throw popupError;
    }
  } catch (error) {
    console.error('Error al iniciar sesión con Google:', error);
    return { error };
  } finally {
    isAuthActionInProgress = false;
  }
};

// Recoge el resultado de signInWithRedirect al volver de Google (móvil). Debe llamarse una
// vez al arrancar la app para poder mostrar un error si el login por redirect falla; si no
// hubo redirect pendiente, devuelve null sin más.
export const getGoogleRedirectResult = async () => {
  if (!auth) return null;
  try {
    const res = await getRedirectResult(auth);
    return res ? res.user : null;
  } catch (error) {
    console.error('Error al procesar el resultado del redirect de Google:', error);
    return { error };
  }
};

/**
 * Fuerza un refresco del token de ID antes de una operación importante contra Firestore. El SDK
 * de Firebase ya refresca el token en segundo plano, pero si el móvil estuvo con la pantalla
 * bloqueada o la pestaña en segundo plano un buen rato (sesión "antigua"), la primera petición al
 * volver puede salir con el token ya caducado, antes de que ese refresco pasivo llegue a tiempo —
 * Firestore la rechaza con permission-denied aunque la sesión sea perfectamente válida. Llamar a
 * esto antes de leer/escribir algo importante evita esa ventana; es barato si el token ya estaba
 * fresco (getIdToken(true) no fuerza una llamada de red si no hace falta renovar).
 */
export const ensureFreshAuthToken = async () => {
  if (!auth?.currentUser) return;
  try {
    await auth.currentUser.getIdToken(true);
  } catch (e) {
    console.error('[ensureFreshAuthToken] No se pudo refrescar el token:', e.message);
  }
};

export const signOutUser = async () => {
  if (auth) {
    await signOut(auth);
  }
};

export const onAuthChange = (cb) => {
  if (auth) {
    return onAuthStateChanged(auth, cb);
  }
  // Sin Firebase configurado no llega nunca un evento de auth, y UserContext espera uno para marcar
  // isLoaded: la app se quedaba en el esqueleto de carga para siempre en vez de degradar a modo
  // invitado (local-first, con lo que haya en localStorage). Se notifica "sin sesión" una vez.
  let cancelled = false;
  queueMicrotask(() => {
    if (!cancelled) cb(null);
  });
  return () => {
    cancelled = true;
  };
};

// Firestore rechaza de forma SÍNCRONA (no como un rechazo de promesa) cualquier escritura que
// contenga un campo con valor `undefined` en cualquier nivel — a diferencia de JSON.stringify
// (que simplemente omite esas claves sin quejarse, por eso localStorage nunca lo delataba). Un
// único campo undefined en un único documento de un batch de cientos aborta el batch entero antes
// de que el manejador de reintentos pueda actuar. Se satura recursivamente aquí, en el punto común
// de escritura, en vez de confiar en que cada llamador construya siempre objetos sin huecos.
function stripUndefinedDeep(value) {
  if (Array.isArray(value)) return value.map(stripUndefinedDeep);
  if (value && typeof value === "object" && !(value instanceof Date)) {
    const out = {};
    Object.entries(value).forEach(([key, v]) => {
      if (v === undefined) return;
      out[key] = stripUndefinedDeep(v);
    });
    return out;
  }
  return value;
}

// Helpers de Firestore
export const saveToCloud = async (path, data) => {
  if (!db) throw new Error("No hay conexión a la base de datos");

  try {
    console.log(`[Firestore] Guardando en: ${path}...`);
    const docRef = doc(db, path);
    await withTimeout(setDoc(docRef, stripUndefinedDeep(data), { merge: true }), "Error de conexión con la base de datos");
    console.log(`[Firestore] Guardado con éxito en: ${path}`);
  } catch (e) {
    console.error(`[Firestore] Error al guardar en ${path}:`, e.message);
    // No relanzamos el error de timeout para no bloquear la UI permanentemente,
    // pero lo registramos.
    if (e.message?.includes("Error de conexión")) {
      console.warn("[Firestore] El guardado continuará en segundo plano (reintentos de Firebase)");
    } else {
      throw e;
    }
  }
};

export const getFromCloud = async (path) => {
  if (!db) return null;
  try {
    const docRef = doc(db, path);
    const snap = await withTimeout(getDoc(docRef));
    return snap.exists() ? snap.data() : null;
  } catch (e) {
    console.error("Error fetching from cloud:", e.message);
    return null;
  }
};

export const updateCloud = async (path, data) => {
  if (!db) return;
  try {
    const docRef = doc(db, path);
    await updateDoc(docRef, data);
  } catch (e) {
    console.error("Error updating cloud:", e);
  }
};

export const deleteFromCloud = async (path) => {
  if (!db) return;
  try {
    await deleteDoc(doc(db, path));
  } catch (e) {
    console.error("Error deleting from cloud:", e);
  }
};

// Margen bajo el límite real de 500 operaciones por batch de Firestore.
const BATCH_CHUNK_SIZE = 450;

const chunkArray = (arr, size) => {
  const chunks = [];
  for (let i = 0; i < arr.length; i += size) chunks.push(arr.slice(i, i + size));
  return chunks;
};

/**
 * El id local de un entrenamiento (Date.now() al terminarlo, o Date.parse(fecha)+índice al
 * importarlo de Hevy) NO incluye nada del usuario. Si dos cuentas distintas importan el mismo
 * CSV (fechas de entrenamiento idénticas), generan exactamente los mismos ids — y como el
 * documento público vivía en `workouts/{id}` a secas, la segunda cuenta intentaba escribir o
 * borrar un documento cuyo dueño real era la otra cuenta. Las reglas de Firestore lo deniegan,
 * y como esas escrituras van en un batch (todo o nada), UN solo id en conflicto tiraba abajo
 * el lote entero sin avisar. Prefijar con el uid hace que la colisión sea imposible.
 */
export const getPublicWorkoutDocId = (uid, localId) => `${uid}_${localId}`;

/**
 * Publica un lote de entrenamientos importados en `workouts/{uid}_{id}` (un documento por
 * entreno) usando batches atómicos de Firestore en vez de un setDoc individual por documento —
 * disparar cientos de escrituras concurrentes es poco fiable (compiten por conexión, se cortan
 * si el usuario navega justo tras "importación completada"). Un batch es una sola petición de
 * red por hasta ~450 documentos. Si aun así Firestore rechaza el lote entero (p.ej. por una
 * regla de seguridad violada en un solo documento), reintentamos cada entrenamiento del lote
 * por separado para no perder los que sí eran válidos.
 *
 * Ya NO escribe el historial agregado en `users/{uid}.completedWorkouts` — ese único campo
 * guardaba el historial COMPLETO de cada usuario como un solo array dentro de un solo documento,
 * contra el límite de 1 MiB por documento de Firestore. Una importación grande (o repetida)
 * podía superarlo y tirar abajo el batch entero — incluidos los propios documentos de
 * `workouts/{uid}_{id}` de este mismo lote, que son pequeños y no tenían motivo para fallar.
 * `getAllUserWorkouts` reconstruye el historial del usuario consultando esta colección
 * directamente (ver refreshData en context/UserContext.js), que ya estaba indexada y en uso
 * para los perfiles públicos (pages/user/[uid].js) — cada entreno es su propio documento, así
 * que no hay ningún límite de tamaño agregado que golpear.
 */
export const bulkSaveWorkoutsToCloud = async (uid, workouts, { userName, userPhoto }) => {
  if (!db) throw new Error("No hay conexión a la base de datos");
  const failures = [];

  const buildDocData = (workout) => stripUndefinedDeep({
    ...workout,
    userId: uid,
    userName,
    userPhoto,
    likes: [],
    commentsList: [],
    isPublic: true,
  });

  const chunks = chunkArray(workouts, BATCH_CHUNK_SIZE);
  if (chunks.length === 0) chunks.push([]);

  for (let i = 0; i < chunks.length; i++) {
    const chunk = chunks[i];
    const batch = writeBatch(db);
    chunk.forEach((workout) => {
      batch.set(doc(db, "workouts", getPublicWorkoutDocId(uid, workout.id)), buildDocData(workout), { merge: true });
    });

    try {
      await batch.commit();
    } catch (error) {
      console.error(`[bulkSaveWorkoutsToCloud] Batch ${i} rechazado, reintentando uno a uno:`, error);
      const results = await Promise.allSettled(
        chunk.map((workout) =>
          setDoc(doc(db, "workouts", getPublicWorkoutDocId(uid, workout.id)), buildDocData(workout), { merge: true })
        )
      );
      results.forEach((r, idx) => {
        if (r.status === 'rejected') {
          console.error(`[bulkSaveWorkoutsToCloud] No se pudo guardar el entrenamiento ${chunk[idx].id}:`, r.reason);
          failures.push({ id: chunk[idx].id, reason: r.reason });
        }
      });
    }
  }

  if (failures.length > 0) {
    const error = new Error(`No se pudieron sincronizar ${failures.length} entrenamientos`);
    error.code = "bulk-workout-write-failed";
    error.failures = failures;
    throw error;
  }
};

/**
 * Todo el historial de entrenos de un usuario, consultando `workouts` por `userId` en vez de
 * leerlo del campo agregado `users/{uid}.completedWorkouts` (retirado, ver nota en
 * bulkSaveWorkoutsToCloud). Sin paginar a propósito — es el reemplazo de aquel array completo
 * para el caché local del propio dueño (context/UserContext.js), que asume el historial entero
 * disponible de una vez, a diferencia de getUserWorkouts (paginado, para ver el perfil de OTRO
 * usuario en pages/user/[uid].js). limit(5000) es solo un tope de cordura, no una paginación
 * real — el índice compuesto (userId + completedAt) ya existe en firestore.indexes.json porque
 * getUserWorkouts ya lo usaba.
 */
export const getAllUserWorkouts = async (userId) => {
  if (!db || !userId) return [];
  const q = query(collection(db, "workouts"), where("userId", "==", userId), orderBy("completedAt", "desc"), limit(5000));
  const snap = await withTimeout(getDocs(q));
  return snap.docs.map((d) => ({ ...d.data(), id: d.id }));
};

/**
 * Borra TODOS los documentos públicos de `workouts` que pertenecen a este usuario, buscándolos
 * en Firestore por el campo `userId` en vez de reconstruir sus ids desde el estado local. A lo
 * largo de varias correcciones (import en batch, luego prefijo de uid en el id) pueden haber
 * quedado documentos con esquemas de id antiguos que el estado local ya no conoce — borrar por
 * id reconstruido nunca puede garantizar que se limpia todo; preguntarle a Firestore qué existe
 * de verdad sí lo garantiza, sea cual sea el esquema con el que se creó cada documento.
 */
export const deleteAllPublicWorkoutsForUser = async (uid) => {
  if (!db || !uid) return;
  const snap = await getDocs(query(collection(db, "workouts"), where("userId", "==", uid), limit(5000)));
  if (snap.empty) return;

  const chunks = chunkArray(snap.docs, BATCH_CHUNK_SIZE);
  for (const chunk of chunks) {
    const batch = writeBatch(db);
    chunk.forEach((d) => batch.delete(d.ref));
    try {
      await batch.commit();
    } catch (error) {
      console.error('[deleteAllPublicWorkoutsForUser] Batch rechazado, reintentando uno a uno:', error);
      const results = await Promise.allSettled(chunk.map((d) => deleteDoc(d.ref)));
      results.forEach((r, idx) => {
        if (r.status === 'rejected') {
          console.error(`[deleteAllPublicWorkoutsForUser] No se pudo borrar ${chunk[idx].id}:`, r.reason);
        }
      });
    }
  }
};

export const addToCollection = async (colPath, data) => {
  if (!db) return null;
  try {
    const docRef = await addDoc(collection(db, colPath), {
      ...data,
      createdAt: new Date().toISOString()
    });
    return docRef.id;
  } catch (e) {
    console.error("Error adding to collection:", e);
    return null;
  }
};

export const uploadProfilePhoto = async (uid, dataUrl) => {
  if (!storage || !uid || !dataUrl) {
    console.warn("[Storage] Faltan parámetros para subir foto:", { uid, dataUrl: !!dataUrl });
    return null;
  }
  
  // Timeout de 15 segundos para la subida
  const timeoutPromise = new Promise((_, reject) => 
    setTimeout(() => reject(new Error("Timeout: La operación tardó demasiado")), 15000)
  );

  try {
    const path = `profilePhotos/${uid}/${Date.now()}`;
    console.log(`[Storage] Subiendo imagen a: ${path}...`);
    const storageRef = ref(storage, path);
    
    const uploadTask = uploadString(storageRef, dataUrl, 'data_url');
    
    await Promise.race([uploadTask, timeoutPromise]);
    
    console.log("[Storage] Imagen subida, obteniendo URL...");
    const url = await getDownloadURL(storageRef);
    console.log("[Storage] URL obtenida:", url);
    return url;
  } catch (e) {
    console.error('[Storage] Error al subir foto de perfil:', e.message);
    // Si falla por CORS o Timeout, devolvemos null para que el perfil no se rompa
    return null;
  }
};

export const getPosts = (callback) => {
  if (!db) return () => {};
  const q = query(collection(db, "posts"), orderBy("createdAt", "desc"), limit(50));
  return onSnapshot(
    q,
    (snapshot) => {
      const posts = snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id }));
      callback(posts);
    },
    // Sin este segundo callback, cualquier error del listener (permission-denied si la sesión de
    // Auth aún no está lista, un hipo de red...) subía como "Uncaught Error in snapshot listener"
    // en vez de degradar con gracia. No tocamos el estado del caller aquí — un fallo puntual no
    // debe vaciar lo que ya se había cargado.
    (error) => console.error('[getPosts] Error en el listener:', error.code || error.message)
  );
};

export const likePost = async (postId, userId) => {
  if (!db || !postId) return;
  const postRef = doc(db, "posts", String(postId));
  const postSnap = await getDoc(postRef);
  if (postSnap.exists()) {
    const postData = postSnap.data();
    const likes = postData.likes || [];
    if (likes.includes(userId)) {
      await updateDoc(postRef, { likes: arrayRemove(userId) });
    } else {
      await updateDoc(postRef, { likes: arrayUnion(userId) });
    }
  }
};

export const addComment = async (postId, comment) => {
  if (!db || !postId) return;
  const postRef = doc(db, "posts", String(postId));
  await updateDoc(postRef, {
    comments: arrayUnion({
      ...comment,
      createdAt: new Date().toISOString()
    })
  });
};

// 🔔 Notificaciones sociales (like/comentario/nuevo seguidor). No exportada: es un detalle
// interno de followUser/likeWorkout/addWorkoutComment, las tres acciones que las disparan. Un
// fallo aquí nunca debe romper la acción social en sí (dar like sigue funcionando aunque la
// notificación no se pueda escribir), así que el error solo se registra en consola.
const createNotification = async (toUserId, data) => {
  if (!db || !toUserId) return;
  try {
    await addDoc(collection(db, "users", toUserId, "notifications"), {
      ...data,
      read: false,
      createdAt: Date.now(),
    });
  } catch (e) {
    console.error('[createNotification] Error:', e);
  }
};

/** Últimas notificaciones del usuario en vivo (like/comentario/seguidor nuevo), más recientes primero. */
export const subscribeToNotifications = (uid, callback) => {
  if (!db || !uid) return () => {};
  const q = query(collection(db, "users", uid, "notifications"), orderBy("createdAt", "desc"), limit(30));
  return onSnapshot(
    q,
    (snapshot) => {
      callback(snapshot.docs.map(d => ({ ...d.data(), id: d.id })));
    },
    // permission-denied es esperable justo al iniciar/cerrar sesión, mientras el token de Auth
    // todavía no está listo para Firestore o authUser ya cambió pero este listener concreto
    // (con el uid anterior) no se ha desuscrito todavía — sin este handler, Firestore lo sube
    // como error no capturado en vez de dejar que el efecto de UserContext.js lo desmonte solo.
    (error) => console.error('[subscribeToNotifications] Error en el listener:', error.code || error.message)
  );
};

export const markNotificationsRead = async (uid, notificationIds) => {
  if (!db || !uid || !notificationIds || notificationIds.length === 0) return;
  const batch = writeBatch(db);
  notificationIds.forEach((id) => batch.update(doc(db, "users", uid, "notifications", id), { read: true }));
  try {
    await batch.commit();
  } catch (e) {
    console.error('[markNotificationsRead] Error:', e);
  }
};

// 👥 Sistema de Seguidores
export const followUser = async (followerId, followedId, actor) => {
  if (!db) return;
  const followerRef = doc(db, "usersPublic", followerId);
  await updateDoc(followerRef, { following: arrayUnion(followedId) });
  if (actor) {
    createNotification(followedId, {
      type: 'follow',
      fromUserId: followerId,
      fromUserName: actor.name || 'Alguien',
      fromUserPhoto: actor.photo || null,
    });
  }
};

export const unfollowUser = async (followerId, followedId) => {
  if (!db) return;
  const followerRef = doc(db, "usersPublic", followerId);
  await updateDoc(followerRef, { following: arrayRemove(followedId) });
};

export const getFollowersCount = async (userId) => {
  if (!db) return 0;
  const q = query(collection(db, "usersPublic"), where("following", "array-contains", userId));
  const snap = await getDocs(q);
  return snap.size;
};

export const getFollowersList = async (userId) => {
  if (!db) return [];
  try {
    const q = query(collection(db, "usersPublic"), where("following", "array-contains", userId));
    const snap = await withTimeout(getDocs(q));
    return snap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
  } catch (e) {
    console.error("Error fetching followers list:", e.message);
    return [];
  }
};

export const getFollowingList = async (userId) => {
  if (!db) return [];
  const userDoc = await getDoc(doc(db, "usersPublic", userId));
  if (userDoc.exists()) {
    const followingIds = userDoc.data().following || [];
    if (followingIds.length === 0) return [];
    
    // Fetch user details for each following ID
    const users = [];
    for (const id of followingIds) {
      const uDoc = await getDoc(doc(db, "usersPublic", id));
      if (uDoc.exists()) users.push({ id: uDoc.id, ...uDoc.data() });
    }
    return users;
  }
  return [];
};

export const searchUsers = async (searchTerm) => {
  if (!db) return [];
  try {
    if (!searchTerm || !searchTerm.trim()) {
      // Usar getDocs simple para evitar problemas de índices en carga inicial
      const q = query(collection(db, "usersPublic"), limit(15));
      const snap = await getDocs(q);
      return snap.docs.map(doc => ({ ...doc.data(), id: doc.id }));
    }
    
    const searchLower = searchTerm.toLowerCase();
    // Búsqueda aproximada usando prefijo
    const q = query(
      collection(db, "usersPublic"),
      where("usernameLowercase", ">=", searchLower),
      where("usernameLowercase", "<=", searchLower + "\uf8ff"),
      limit(10)
    );
    const snap = await getDocs(q);
    
    if (snap.empty) {
      // Fallback: buscar en todos (si son pocos) o simplemente devolver vacio
      // Según el usuario, prefiere getDocs directo si falla
      const allSnap = await getDocs(collection(db, "usersPublic"));
      return allSnap.docs
        .map(doc => ({ ...doc.data(), id: doc.id }))
        .filter(u => u.usernameLowercase?.includes(searchLower))
        .slice(0, 10);
    }
    
    return snap.docs.map(doc => ({ ...doc.data(), id: doc.id }));
  } catch (e) {
    console.error("Error searching users:", e);
    try {
      // Último recurso: getDocs sin filtros (compatible con las nuevas reglas)
      const allSnap = await getDocs(collection(db, "usersPublic"));
      return allSnap.docs.map(doc => ({ ...doc.data(), id: doc.id })).slice(0, 10);
    } catch (err) {
      return [];
    }
  }
};

export const getAllPublicUsers = async () => {
  if (!db) return [];
  const q = query(collection(db, "usersPublic"), limit(20));
  const snap = await getDocs(q);
  return snap.docs.map(doc => ({ ...doc.data(), id: doc.id }));
};

/**
 * Feed paginado por cursor. `cursor` es el QueryDocumentSnapshot devuelto como
 * `cursor` en la página anterior (null para la primera página).
 * Devuelve `hasMore` para saber si mostrar "cargar más" sin necesitar un count aparte.
 */
export const getWorkoutsFeed = async (userIds, { pageSize = 15, cursor = null } = {}) => {
  if (!db || !userIds || userIds.length === 0) return { workouts: [], cursor: null, hasMore: false };
  const constraints = [
    collection(db, "workouts"),
    where("userId", "in", userIds.slice(0, 10)), // Firestore limit
    orderBy("completedAt", "desc"),
  ];
  if (cursor) constraints.push(startAfter(cursor));
  constraints.push(limit(pageSize + 1)); // +1 para saber si hay siguiente página sin otra query

  const q = query(...constraints);
  const snap = await getDocs(q);
  const hasMore = snap.docs.length > pageSize;
  const pageDocs = hasMore ? snap.docs.slice(0, pageSize) : snap.docs;

  return {
    workouts: pageDocs.map(d => ({ ...d.data(), id: d.id })),
    cursor: pageDocs.length > 0 ? pageDocs[pageDocs.length - 1] : null,
    hasMore,
  };
};

/** Feed paginado por cursor, misma forma de retorno que getWorkoutsFeed. */
export const getUserWorkouts = async (userId, { pageSize = 20, cursor = null } = {}) => {
  if (!db || !userId) return { workouts: [], cursor: null, hasMore: false };
  const constraints = [
    collection(db, "workouts"),
    where("userId", "==", userId),
    orderBy("completedAt", "desc"),
  ];
  if (cursor) constraints.push(startAfter(cursor));
  constraints.push(limit(pageSize + 1));

  const q = query(...constraints);
  const snap = await getDocs(q);
  const hasMore = snap.docs.length > pageSize;
  const pageDocs = hasMore ? snap.docs.slice(0, pageSize) : snap.docs;

  return {
    workouts: pageDocs.map(d => ({ ...d.data(), id: d.id })),
    cursor: pageDocs.length > 0 ? pageDocs[pageDocs.length - 1] : null,
    hasMore,
  };
};

/**
 * Total de entrenos públicos de un usuario. Usa una query de agregación (`count()`) en vez de
 * `getDocs(...).size` para no tener que descargar todos los documentos solo para contarlos —
 * importante aquí porque, a diferencia de seguidores/seguidos, el historial de entrenos de
 * alguien puede crecer a cientos y este contador se pide independientemente de la paginación
 * de `getUserWorkouts`.
 */
export const getUserWorkoutsCount = async (userId) => {
  if (!db || !userId) return 0;
  const snap = await getCountFromServer(query(collection(db, "workouts"), where("userId", "==", userId)));
  return snap.data().count;
};

export const likeWorkout = async (workoutId, userId, actor) => {
  if (!db || !workoutId) return;
  const ref = doc(db, "workouts", String(workoutId));
  const snap = await getDoc(ref);
  if (snap.exists()) {
    const data = snap.data();
    const likes = data.likes || [];
    if (likes.includes(userId)) {
      await updateDoc(ref, { likes: arrayRemove(userId) });
    } else {
      await updateDoc(ref, { likes: arrayUnion(userId) });
      // Solo se notifica al DAR like (no al quitarlo), y nunca a uno mismo si te das like a tu
      // propio entreno.
      if (actor && data.userId && data.userId !== userId) {
        createNotification(data.userId, {
          type: 'like',
          fromUserId: userId,
          fromUserName: actor.name || 'Alguien',
          fromUserPhoto: actor.photo || null,
          workoutId,
        });
      }
    }
  }
};

export const addWorkoutComment = async (workoutId, comment) => {
  if (!db || !workoutId) return;
  const ref = doc(db, "workouts", String(workoutId));
  await updateDoc(ref, {
    commentsList: arrayUnion({
      ...comment,
      createdAt: comment.createdAt || Date.now()
    })
  });

  const snap = await getDoc(ref);
  if (snap.exists()) {
    const data = snap.data();
    if (data.userId && data.userId !== comment.authorId) {
      createNotification(data.userId, {
        type: 'comment',
        fromUserId: comment.authorId,
        fromUserName: comment.authorName || 'Alguien',
        fromUserPhoto: comment.authorPhoto || null,
        workoutId,
        commentText: comment.text || '',
      });
    }
  }
};
