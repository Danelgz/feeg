...
/** Borra en batch las copias públicas (`workouts/{uid}_{id}`) de la lista de ids locales dada. */
export const bulkDeleteWorkoutsFromCloud = async (uid, localWorkoutIds) => {
  if (!db || !localWorkoutIds || localWorkoutIds.length === 0) return;
  const chunks = chunkArray(localWorkoutIds, BATCH_CHUNK_SIZE);
  for (const chunk of chunks) {
    const batch = writeBatch(db);
    chunk.forEach((id) => batch.delete(doc(db, "workouts", getPublicWorkoutDocId(uid, id))));
    try {
      await batch.commit();
    } catch (error) {
      // Igual que en bulkSaveWorkoutsToCloud: un solo documento problemático tira abajo el
      // lote entero y ninguno se borra. Reintentamos uno a uno para borrar todos los que se
      // pueda en vez de ninguno.
      console.error('[bulkDeleteWorkoutsFromCloud] Batch rechazado, reintentando uno a uno:', error);
      const results = await Promise.allSettled(
        chunk.map((id) => deleteDoc(doc(db, "workouts", getPublicWorkoutDocId(uid, id))))
      );
      results.forEach((r, idx) => {
        if (r.status === 'rejected') {
          console.error(`[bulkDeleteWorkoutsFromCloud] No se pudo borrar ${chunk[idx]}:`, r.reason);
        }
      });
    }
  }
...
/**
 * Best-effort: borra las claves antiguas sin prefijo de uid (`workouts/{id}` a secas), por si
 * alguno de estos entrenamientos se había publicado antes de introducir getPublicWorkoutDocId.
 * Solo puede borrar los que de verdad son tuyos (la regla de seguridad lo exige); los fallos de
 * los demás (huérfanos de otra cuenta con el mismo id, o que ya no existen) se ignoran uno a
 * uno sin tirar abajo el resto.
 */
export const bulkDeleteRawWorkoutDocs = async (localWorkoutIds) => {
  if (!db || !localWorkoutIds || localWorkoutIds.length === 0) return;
  const results = await Promise.allSettled(
    localWorkoutIds.map((id) => deleteDoc(doc(db, "workouts", String(id))))
  );
  results.forEach((r, idx) => {
    if (r.status === 'rejected') {
      console.error(`[bulkDeleteRawWorkoutDocs] No se pudo borrar ${localWorkoutIds[idx]}:`, r.reason);
    }
  });
};
