...
function BodyHeatmapWrapper({ muscleStats, t, isDark, isMobile }) {
  const { counts, getIntensity, getColor } = muscleStats;
  const [manualLevels, setManualLevels] = useState({});

  const handleMuscleClick = (muscle) => {
    const currentLevel = manualLevels[muscle] ?? getIntensity(counts[muscle] || 0);
    setManualLevels(prev => ({ ...prev, [muscle]: (currentLevel + 1) % 5 }));
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", gap: "20px", alignItems: "flex-start" }}>
      ...
      <div style={{ flex: 1, width: "100%" }}>
        <h3 style={{ fontSize: "0.9rem", color: isDark ? "#fff" : "#333", marginBottom: 15 }}>Estadísticas Detalladas</h3>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
          {Object.entries(counts).sort((a,b) => b[1] - a[1]).map(([m, val]) => {
            const level = manualLevels[m] ?? getIntensity(val);
            return (
              <div key={m} 
              ...
              >
                <div style={{ fontSize: "0.7rem", color: isDark ? "#888" : "#999", textTransform: "uppercase" }}>{t(m) || m}</div>
                <div style={{ fontSize: "1rem", fontWeight: "bold", color: isDark ? "#fff" : "#000" }}>{val} <span style={{ fontSize: "0.7rem" }}>SERIES</span></div>
              </div>
            );
          })}
        </div>
