...
const WORD_TRANSLATIONS = {
  ...
  bar: "barra", bars: "barra", plate: "disco", plates: "disco", ezbar: "barra", "ez-bar": "barra",
  lat: "jalon", lats: "jalon", pulley: "polea", pulleys: "polea", rope: "cuerda",
  straight: "recto", vbar: "v", "v-bar": "v", tbar: "t", "t-bar": "t",
  loaded: "cargado", support: "apoyo", supported: "apoyo", pressdown: "extension", pushdown: "extension", pushdowns: "extension",
  adductor: "aductor", adduction: "aduccion", abductor: "abductor", abduction: "abduccion",
  clamshell: "almeja", "cable-pull-through": "pullthrough",
  barbell: "barra", barbells: "barra", barra: "barra",
  dumbbell: "mancuerna", dumbbells: "mancuerna", mancuerna: "mancuerna", mancuernas: "mancuerna",
  machine: "maquina", maquina: "maquina", maquinas: "maquina",
  cable: "polea", cables: "polea", polea: "polea",
  bodyweight: "corporal", corporal: "corporal",
};

const STOP_WORDS = new Set(["a", "al", "and", "con", "de", "del", "en", "for", "la", "las", "of", "the", "to", "un", "una", "y"]);
...
const ALIASES = {
  ...
  "machine leg press": "Prensa de Piernas",
  "barbell squat": "Sentadilla (Barra)",
  "dumbbell shoulder press": "Press de Hombros (Mancuerna)",
  "cable row": "Remo Sentado con Cable",
...
function normalizeToken(token) {
  const translated = WORD_TRANSLATIONS[token] || token;
  if (translated.length > 4 && translated.endsWith("s")) return translated.slice(0, -1);
  return translated;
}
...
function extractEquipmentHint(originalName) {
  const tokens = tokenize(normalize(originalName));
...
let builtInCatalogIndex = null;
function getBuiltInCatalogIndex() {
  if (builtInCatalogIndex) return builtInCatalogIndex;
  builtInCatalogIndex = [];
  Object.entries(exercisesList).forEach(([group, list]) => {
    list.forEach((ex) => {
      builtInCatalogIndex.push({
        name: ex.name,
        group,
        equipment: ex.equipment,
        normalizedName: normalize(ex.name),
        tokens: new Set(translateTokens(tokenize(normalize(ex.name)))),
      });
    });
  });
...
function getCustomCatalogIndex() {
  if (typeof window === "undefined") return [];
  try {
    const stored = JSON.parse(localStorage.getItem("customExercises") || "{}");
    return Object.entries(stored).flatMap(([group, list]) =>
      (Array.isArray(list) ? list : []).map((ex) => ({
        name: ex.name,
...
function getCatalogIndex() {
  return [...getBuiltInCatalogIndex(), ...getCustomCatalogIndex()];
}

function findExactCatalogMatch(name) {
  const normalizedName = normalize(name);
  return getCatalogIndex().find((candidate) => candidate.normalizedName === normalizedName) || null;
}

let normalizedAliasMap = null;
function getNormalizedAliasMap() {
  if (normalizedAliasMap) return normalizedAliasMap;
  normalizedAliasMap = Object.fromEntries(Object.entries(ALIASES).map(([key, value]) => [normalize(key), value]));
...
export function matchExerciseName(foreignName) {
  ...
  // Un nombre idéntico al catálogo ya comparte identidad: no necesita fuzzy matching ni revisión.
  // Se comprueba antes de aliases para que una coincidencia literal nunca se redirija a otro ejercicio.
  const exact = findExactCatalogMatch(foreignName);
  if (exact) return { name: exact.name, group: exact.group, score: 1, method: "exact" };

  const userAliases = getAliasMap();
  if (userAliases[key]) {
    const catalogHit = findExactCatalogMatch(userAliases[key].name);
    return { ...userAliases[key], score: 1, method: "user_alias", group: catalogHit?.group || userAliases[key].group };
  }

  const aliasTarget = getNormalizedAliasMap()[key];
  if (aliasTarget) {
    const catalogHit = findExactCatalogMatch(aliasTarget);
  ...
  getCatalogIndex().forEach((candidate) => {
    ...
    const overlap = [...queryTokens].filter((token) => candidate.tokens.has(token)).length;
    const queryCoverage = queryTokens.size ? overlap / queryTokens.size : 0;
    const candidateCoverage = candidate.tokens.size ? overlap / candidate.tokens.size : 0;
    // Cubre nombres de otras apps que añaden contexto (p. ej. "plate loaded chest press") sin
    // permitir que un solo token genérico gane por accidente.
    score = Math.max(score, queryCoverage * 0.82 + candidateCoverage * 0.18);
    if (equipmentHint && candidate.equipment === equipmentHint) score = Math.min(1, score + 0.15);
    if (equipmentHint && candidate.equipment && candidate.equipment !== equipmentHint) score = Math.max(0, score - 0.08);
    ranked.push({ name: candidate.name, group: candidate.group, score, method: "fuzzy" });
  });
  ...
  return { ...best, ambiguous: !!second && best.score - second.score < 0.08 };
}
 ...
 * usuario). Las coincidencias difusas nunca se aplican solas: `pending` las lleva con una
 * sugerencia para que el usuario confirme, porque dos ejercicios parecidos pueden no ser iguales.
 */
